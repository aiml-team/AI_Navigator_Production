...
# ──────────────────────────────────────────────────────────────────────────
#  OKTA SSO DISABLED
#  The entire Okta/SAML flow has been commented out per request.
#  Login now uses the simple email-based identifier at /api/auth/identify
#  (see routes/auth_routes.py). To re-enable Okta SSO later, uncomment this
#  whole module and also restore the saml_router include in routes/__init__.py.
# ──────────────────────────────────────────────────────────────────────────
#
# import os
...
#     against our Azure SQL DB, store them in the server-side session,
#     then redirect to /.
#     """
...
#
#     # identify_user() internally canonicalizes the email and matches
#     # any alias form (e.g. @bs.nttdata.com vs @nttdata.com) so users
#     # keep their pre-Okta admin status & history.
#     try:
...
# ──────────────────────────────────────────────────────────────────────────
# Stub router so any lingering `from routes.saml_routes import router`
# imports continue to work without raising. It exposes no endpoints.
# Safe to delete once routes/__init__.py no longer imports this module.
# ──────────────────────────────────────────────────────────────────────────
